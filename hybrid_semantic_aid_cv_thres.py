...
logging.basicConfig(
    filename="aid_classifier_cv.log",
    level=logging.INFO,
...
def build_features_and_labels(df_survey, df_kw, model):
    ...
    aid_codes = sorted(df_kw['aid_type_code'].unique())
    grouped_keywords = df_kw.groupby('aid_type_code')
    ...
    MAX_KEYWORDS = df_kw.groupby('aid_type_code').size().max()
    ...
    y = df_survey[["lbl_01_aid_type", "lbl_02_aid_type", "lbl_03_aid_type"]].values.astype(np.int32)
...
def robustness_split_and_evaluate(X_os, y_os, df_survey_os, best_estimators, threshold_map, default_threshold=None, folds=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_os, stratify_label)):
        ...
        probas = np.column_stack([
            clf.predict_proba(X_test)[:, 1] if len(clf.classes_) > 1 
            else np.full(X_test.shape[0], 1.0 if clf.classes_[0] == 1 else 0.0)
            for clf in best_estimators
        ])
        preds = apply_threshold(probas, threshold_map, default_threshold)
...
def crossval_with_fixed_threshold(X, y, df_survey, use_tuned_threshold=True, manual_thresholds=None, folds=5):
    ...
    for i in range(y.shape[1]):
        ...
        log(f"[THRESH CHECK] Label {i}: pos={np.sum(y_os[:, i]==1)}, neg={np.sum(y_os[:, i]==0)}")
        clf.fit(X_os, y_os[:, i])
        best_estimators.append(clf)

        y_prob = clf.predict_proba(X_os)[:, 1] if len(clf.classes_) == 2 else np.zeros(X_os.shape[0])
        best_thresh, fpr, tpr, _ = find_best_threshold(y_os[:, i], y_prob)
        auc_val = auc(fpr, tpr) if fpr is not None else float('nan')

        fallback = False
        if fpr is None or tpr is None or np.isnan(auc_val):
            log(f"[WARN] Label {i}: AUC or ROC is invalid. Falling back to manual threshold.")
            fallback = True
        elif best_thresh >= 0.9 or best_thresh <= 0.1:
            log(f"[WARN] Label {i}: Threshold {best_thresh:.3f} looks extreme. Using manual fallback.")
            fallback = True

        if fallback:
            best_thresh = manual_thresholds.get(i, 0.5)
...
def save_threshold_to_db(engine, task_name, label_index, label_name, threshold, weight):
    with engine.connect() as conn:
        conn.execute(
            ...
            (task_name, label_index, label_name, threshold, class_weight, created_at)
            VALUES (%s, %s, %s, %s, %s, GETDATE())
            """,
            (task_name, label_index, label_name, threshold, weight)
        )
...
if __name__ == "__main__":
    ...
    df_kw = pd.read_sql("""
        SELECT *
        FROM NLP_DOGSTRUST.dbo.t_key_report_aidtype where flag=1
    """, engine)
    ...
    manual_thresholds = {0: 0.6, 1: 0.6, 2: 0.6}
    crossval_with_fixed_threshold(X, y, df_survey_full, use_tuned_threshold=True, manual_thresholds=manual_thresholds)
